...
export class GhibliAPI extends RESTDataSource {
  override baseURL = "https://ghibliapi.vercel.app/";
  ...
  async getFilms(find?: FilmFind, sort?: string): Promise<Film[]> {
    ...
    if (
      sort &&
      films.length &&
      (sort in films[0] || sort.substring(1) in films[0])
    ) {
      if (sort.charAt(0) === "!") {
  ...
  async getAllCreators(): Promise<Creator[]> {
    ...
    return films.reduce((creators, film) => {
      ...
      const directorMatch = creators.find(
        (creator) => creator.name === director
      );
      if (!directorMatch) {
        creators.push({
          name: director,
      ...
      const producerMatch = creators.find(
        (creator) => creator.name === producer
      );
      if (!producerMatch) {
        creators.push({
          name: producer,
  ...
  async getDirectors(find?: CreatorFind, sort?: string): Promise<Creator[]> {
    ...
    if (find?.film) {
      directors = directors.filter((director) =>
        director.directed.find((film) => film.title === find.film)
      );
    }
    ...
    if (
      sort &&
      directors.length &&
      (sort in directors[0] || sort.substring(1) in directors[0])
    ) {
      if (sort.charAt(0) === "!") {
        const sortKey = sort.substring(1) as keyof Creator;
        directors = directors.sort(
          this.reverseCompareFunction<Creator>(sortKey)
        );
      } else {
  ...
  async getProducers(find?: CreatorFind, sort?: string): Promise<Creator[]> {
    ...
    if (find?.film) {
      producers = producers.filter((producer) =>
        producer.produced.find((film) => film.title === find.film)
      );
    }
    ...
    if (
      sort &&
      producers.length &&
      (sort in producers[0] || sort.substring(1) in producers[0])
    ) {
      if (sort.charAt(0) === "!") {
        const sortKey = sort.substring(1) as keyof Creator;
